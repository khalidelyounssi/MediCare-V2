...
function renderAppointments() {
    const appointments = getAppointments().sort((a, b) => new Date(a.date) - new Date(b.date));
    appointmentsList.innerHTML = '';
    
    if (appointments.length === 0) {
        appointmentsList.innerHTML = '<p class="text-gray-500 dark:text-gray-400" id="no-appointments">Aucun rendez-vous planifié.</p>';
        return;
    }
    ...
    appointments.forEach(app => {
        ...
        appElement.className = 'appointment-card bg-white dark:bg-gray-700 p-5 rounded-xl shadow-lg flex justify-between items-center transition-shadow duration-300 hover:shadow-xl border-l-4 border-secondary dark:border-primary';
...
form.addEventListener('submit', (e) => {
    ...
    if (!form.checkValidity()) {
        validationMessage.textContent = 'Veuillez remplir tous les champs correctement.';
